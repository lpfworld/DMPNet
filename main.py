...
def main():
    net = DSNet('')
    net.cuda()
    # train_path = '/home/lpf/PycharmProjects/DMPNet/data/sha_preprocessed/train/'
    # test_path = '/home/lpf/PycharmProjects/DMPNet/data/sha_preprocessed/test/'


    train_loader, test_loader = get_loader(train_path, test_path, 8)
    save_path = "/home/lpf/PycharmProjects/DMPNet/models/UCF_QNRF_best_model.pth"
    epochs = 500
    
    optimizer = torch.optim.Adam(net.parameters(), lr=1e-5, weight_decay=5e-4)
    criterion = nn.MSELoss()
    best_mae, _ = val(net, test_loader)
    logger = getLogger('logs/shanghaia_Adam_1e-5.txt')
